...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $query = mysqli_query($conn, "DELETE FROM packages WHERE id='$id'");
    ...
    <tbody>
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($query)) {
                ...
                echo '<a href="updatepackage.php?id=' . $row['id'] . '" class="action-btn update-btn"><i class="fas fa-edit"></i></a>';
                echo '<a href="view package.php?delete=' . $row['id'] . '" class="action-btn delete-btn" onclick="return confirm(\'Are you sure you want to delete this package?\')"><i class="fas fa-trash"></i></a>';
                echo '</td>';
                echo '</tr>';
                $i++;
            }
